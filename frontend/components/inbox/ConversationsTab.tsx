...
export function ConversationsTab({
  ...
  /** Back to the list — only offered below `md`, where the list and the thread take turns. */
  onBack: () => void;
}) {
  ...
  return (
    <div className="flex h-[calc(100vh-13rem)] min-h-[28rem] overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
      ...
      <aside
        ...
        className={`${open ? "hidden md:flex" : "flex"} w-full flex-shrink-0 flex-col border-slate-200 md:w-80 md:border-l`}
      >
        {list?.error && (
          <div className="p-3">
      ...
      <div className={`${open ? "flex" : "hidden md:flex"} min-w-0 flex-1 flex-col`}>
        {open && (
          <button
            type="button"
            onClick={onBack}
